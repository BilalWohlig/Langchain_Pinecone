...
const validationSchema = {
  type: 'object',
  required: true,
  properties: { indexName: { type: 'string', required: true, minLength: 3 }, namespace: { type: 'string', required: true, minLength: 3 } }
}
const validation = (req, res, next) => {
  return validationOfAPI(req, res, next, validationSchema, 'body')
}
const deleteAllVectors = async (req, res) => {
  try {
    const deletedVectors = await Pinecone.deleteAllVectorsFromNamespace(req.body.indexName, req.body.namespace)
    res.sendJson({ type: __constants.RESPONSE_MESSAGES.SUCCESS, data: deletedVectors })
  } catch (err) {
    return res.sendJson({
...
router.post('/deleteAllVectors', validation, deleteAllVectors)
